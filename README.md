...
現在は MVP v0.1 を土台に、Phase 2 以降の拡張として 7アイドル・12施設・歌アップグレード・アイテム・記録・廻 v1 に対応しています。
...
- 加入済みアイドルによるパッシブ効果
- 未解放アイドルの解放条件表示
- 施設「路地裏ステージ」「ネオン掲示板」「薄明通り案内所」「仮設配信ブース」「記憶図書館」「録音保管庫」「古い放送室」「地下広場」「名前の記録壁」「地下礼拝堂」「地下通路修復区画」「再固定中枢」
- 施設ごとの解放条件と強化
- 施設による灯るさの自動生産
...
- v1〜v10 セーブデータから v11 セーブデータへの移行
- 最終保存時刻に基づく簡易オフライン報酬
- コンテンツ定義の整合性チェック
...
- 地下礼拝堂 Lv 8 かつ「礼拝堂のハーモニー」取得後、地下通路修復区画が解放される。
- 地下通路修復区画 Lv 1 到達後、継ノ端 桜子が解放される。
- 継ノ端 桜子を注目アイドルに切り替えられる。
- 歌タブを開ける。
- 路地裏ステージ Lv 5 かつ 80 灯るさで「路地裏のイントロ」を取得できる。
- 「路地裏のイントロ」取得後、ライブ1回の獲得灯るさが 2 になる。
- 地下礼拝堂 Lv 2 かつ 25000 灯るさで「礼拝堂のハーモニー」を取得できる。
- 地下礼拝堂 Lv 8 かつ 90000 灯るさで「薄明のコーラス」を取得できる。
...
- 再固定中枢 Lv1 到達後、廻タブから廻を実行できる。
- 初回廻後、記憶断片と購入済み廻後バフがリロード後も残る。
...
window.__NEON_DEBUG__.setSaveForTest({
  ...
  idols: {
    otowaAkari: { bond: 0, eventIdsRead: [], joined: true },
    asagiriYui: { bond: 0, eventIdsRead: [], joined: true },
    mizukiShino: { bond: 0, eventIdsRead: [], joined: true }
  },
  items: {
