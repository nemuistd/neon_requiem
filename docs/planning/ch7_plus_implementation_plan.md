...
2026-05-26 時点で、M9 を優先実装ラインへ繰り上げ、Ch.7、Ch.8、Ch.9 起点、Ch.9 収束補強、`records_draft.md` 実装可能分整理の最小縦切りを実装済み。
...
- 歌 `deepLayerSilence` / 「深層の静謐」。
- アイテム `coverlessObservationLog` / 「観測記録用の表紙のない日誌」。
- 記録「表紙のない観測日誌・抜粋」。
- 記録「深層観測所・静寂測定」。
- 澪の Ch.7 由来2件目イベント `kasumiyamaMio.oneSongPerDay`。

受け入れ条件:

- Ch.7の深層感を補うが、Ch.8の祈念工学解読へ踏み込みすぎない。
- surface UIで「聖歌」を通常システム名として軽く扱わない。deep / technical の記録や設定文脈では、崩壊前の祈念工学上の儀式的・危険な形式として必要に応じて扱う。
- deepタグ倍率が過剰に積み上がらない。
- 新しい保存フィールド、`SAVE_VERSION` 変更、Requirement 型追加、Effect 型追加、外部依存追加は行わない。
...
- 歌 `fragmentMelody` / 「断片の旋律」。
- アイテム `sortedEngineeringFragment` / 「工学文書の断片（整理済み）」。
- 記録「旧評議会・記録の断片」。
- 記録「封印された記憶について」。
- 澪の交流20相当イベント `kasumiyamaMio.knownMachine`。
- 皐月の交流20相当イベント `nanashiroSatsuki.translationEcho`。

受け入れ条件:

- 主人公の旧計画関与は示唆に留める。
- 工学記録と夢の一致を、断定ではなく違和感として扱う。
- 既存の「祈念工学・記録断片」と「実験跡地・現地調査報告」は重複追加しない。
- 新しい保存フィールド、`SAVE_VERSION` 変更、Requirement 型追加、外部依存追加は行わない。
...
### P-M9-RECORD-28: records_draft 実装可能分整理

状態: 実装済み。

実装内容:

- `docs/fiction/records_draft.md` の Ch.6〜Ch.9 残草稿を現行仕様と照合した。
- 記録「バインダー封書・第一行」「廻後の路地裏・微細な変化」「廻・二度目の記録」「深層区画・設計図との差異」「施設ログ・認知固定率の変化」「観測者数・閾値付近」「主人公・最初の記憶の断片」「廻と記憶の関係について」を追加した。
- `RECORD_CONTENT_VERSION` を `18` に更新し、今回追加記録だけを最新扱いにした。
- `records_draft.md` に実装状況一覧と各草稿のステータスを追加し、保留分は `docs/product/40_task_backlog.md` の `records_draft 保留記録` へ転記した。

受け入れ条件:

- 主人公の正体、霞の意志、旧計画の全貌を確定しない。
- 記憶断片を常時生産する扱いへ変更しない。
- 新しい保存フィールド、`SAVE_VERSION` 変更、Requirement 型追加、Effect 型追加は行わない。
