...
2026-05-25 時点で、M9 を優先実装ラインへ繰り上げ、Ch.7 の最小縦切りを実装済み。
M8 の残り交流イベントや廻後バランス調整は保留ではなく、Ch.7 体験補強または Ch.8 着手と合わせて再判断する。

## 優先判断

次に実装する最小単位は、Ch.7 の `深層観測所` と `霞山 澪` に絞る。
...
- 通しバランス調整より先に、廻後の次目標を1つ画面に出す方が、初回廻後の手触りを確認しやすい。
...
| `engineeringArchive` | `deepLayerObservatory` Lv5 |
| `prayerEngineeringRuins` | `engineeringArchive` Lv5 |
