...
- fiction 正史上の Ch.7 以降施設、霞山 澪、七城 皐月、白霧 燐、Ch.9 収束に入らない。
...
- 左側の注目アイドル選択から未加入アイドルをクリックした場合も、そのアイドルへ声をかけて加入できる導線を検討する。
...
1. 左のアイドル一覧から加入 / 詳細確認へ入る導線を固める。
2. アイドル詳細で、交流イベントやアイドル由来の記録を読める表示形式を検討する。
3. 廻清算を専用画面として扱い、通常進行へ戻る導線の強さを調整する。
4. 廻情報をタブとして残すか、バインダー / 状態確認 / 清算画面などへ分散整理するかを決める。
5. 記録タブの分類と、アイドル詳細側へ移した方がよい読み物の境界を決める。
6. 廻後バフと施設解放バランスは、通しプレイの観察後に数値調整する。
