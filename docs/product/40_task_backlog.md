...
2026-05-25時点で、以下のタスク候補は実装済み。現在仕様は [Current Spec](../current_spec.md) を参照する。

以下は、廻 v1 実装後の改善単位として整理した項目である。詳細仕様は [廻本体 仕様メモ](../planning/meguri_system_spec.md) と [アイドル拡張設計](../planning/idol_roster_expansion_design.md) を参照する。
...
- 廻実行直後が清算中であることを、プレイヤーが迷わず理解できるようにする。

対象範囲:

- `pendingSettlement` 中の専用画面またはフル画面状態。
- 記憶断片獲得、購入可能バフ、購入済みバフ、追記が増えた記録の表示。
- 清算終了ボタン、または通常進行へ戻る時の明示的な案内。
- リロード後も清算中なら清算画面へ戻る保存復元。
...
- `P-M4-MEGURI-08` と仕様上は近いため、同一実装者が続けて行うのが望ましい。

Write scope:

- `src/` の廻 UI、状態遷移、保存復元。
- 必要なら `docs/current_spec.md` と `docs/manual_qa_checklist.md`。

受け入れ条件:

- 廻実行後、清算中であることが画面上明確である。
- バフ購入可能な期間と、清算終了条件が分かる。
- 清算終了後は通常プレイ中にバフを追加購入できない。
- リロードしても `pendingSettlement` 中なら清算画面へ戻る。
...
- `npm.cmd run test`
- in-app Browser で廻実行、清算画面、清算終了、リロード復元を確認する。

### P-M4-MEGURI-08: 記憶断片の次断片ゲージ

目的:

- プレイヤーが、今すぐ廻るか、もう少し灯るさを稼ぐか判断できるようにする。

対象範囲:

- 廻前確認画面の獲得見込み表示。
- 次の記憶断片までの必要灯るさ。
- 進捗ゲージと「あと X 灯るさで +1 記憶断片」の表示。
- プレビュー、ゲージ、実付与を同じ計算関数から出すテスト。
...
- 2番目。`P-M4-MEGURI-07` の直後。

並列可否:

- UI と計算関数に触るため、廻 UI を触るタスクとの同時編集は避ける。
...
- `npm.cmd run build`
- `npm.cmd run test`
- in-app Browser で廻前確認、灯るさ増加後のゲージ更新、廻実行を確認する。

### P-M4-RECORD-09: 廻後の記録通知ポリシー調整

目的:

- 廻後の記録通知を、本文変更や追記追加がある時だけ出るものにする。
...
- 記録 UI と保存状態に触るため、同じ箇所の改修とは並列不可。

Write scope:

- `src/` の記録状態、記録 UI、保存移行、テスト。
- 必要なら `docs/current_spec.md`。

受け入れ条件:

- 廻で引き継いだだけの既読記録には不要な通知が出ない。
- 新しい追記が出た記録には通知が出る。
- 追記を読むと追記未読が消える。
- 本文既読と追記既読が保存・復元される。
...
- `npm.cmd run build`
- `npm.cmd run test`
- in-app Browser で記録追記表示、未読バッジ、リロード復元を確認する。

### P-M3-IDOL-10: アイドル加入アクションと効果発動分離

目的:

- アイドルの効果解放を、条件達成時の自動発動ではなく、プレイヤーの能動的な加入操作に結びつける。

対象範囲:

- 条件達成済みと加入済みの状態分離。
- 「声をかける」「プロデュースする」などの加入操作。
- 加入後の効果発動とメッセージ。
- 既存セーブで解放済みアイドルを加入済みに移行する互換処理。

依存タスク:

- 現在のアイドル解放条件と効果集計を確認すること。

推奨実行順:

- 4番目。廻清算 UI と記録通知の方針が固まった後。

並列可否:

- アイドル UI、効果計算、保存移行に触るため、関連タスクとの並列編集は避ける。

Write scope:

- `src/` のアイドル状態、効果集計、UI、保存移行、テスト。
- 必要なら `docs/current_spec.md`。

受け入れ条件:

- 条件達成済みでも未加入ならアイドル効果は発動しない。
- 加入操作後に効果が発動する。
- 初期アイドルは新規開始時点で加入済みである。
- 既存セーブの解放済みアイドルは、移行後も効果を失わない。
- 文言が所有や搾取に見えない。
...
- `npm.cmd run check:content`
- `npm.cmd run test`
- in-app Browser で条件達成、加入操作、効果発動、リロード復元を確認する。

### P-INTEGRATE-11: 廻 v2 / アイドル加入統合QA

目的:

- 廻清算、記憶断片ゲージ、記録通知、アイドル加入が、プレイヤーの判断と導線として矛盾なくつながっているか確認する。
...
- 必要に応じて `docs/current_spec.md`、`docs/manual_qa_checklist.md`、軽微な UI 文言。

受け入れ条件:

- 初回廻前の判断、廻実行、清算、清算終了、再進行が迷わずつながる。
- 記録通知が追記や本文変化に限定されている。
- アイドル効果が加入操作と結びついている。
- モバイル幅で主要表示が崩れない。
...
- `npm.cmd run check:content`
- `npm.cmd run test`
- in-app Browser で初回廻から清算終了後の再進行までを確認する。

実施結果:

- 清算画面に、追記が増えた記録の控えと「通常進行へ戻って記録を見る」導線を追加した。
- 廻前ゲージ、廻実行、清算、廻後バフ購入、追記通知、記録確認、通常進行復帰、アイドル加入を統合確認した。
- `npm.cmd run test`、`npm.cmd run check:content`、`npm.cmd run build` が成功した。
- Browser プラグインは利用できなかったため、Playwright CLI で `http://localhost:5175/` を確認した。
