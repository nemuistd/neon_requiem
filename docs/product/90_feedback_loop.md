...
6. 必要な場合のみ `50_parallel_execution_plan.md`
7. 実装タスク
...
- 同じタスク群を複数の作業者や worktree で同時に進める時。
- タスクのwrite scopeが重なり、並列実行が危うい時。
- 並列ユニットの成果をどの順で合流すべきか判断できない時。
- 実装中に、担当ユニットを分けるべき作業が見つかった時。

並列実行がない通常作業では、`40_task_backlog.md` まで整理されていればよい。完了した並列計画は現役タスクに見えないよう、`archive/` へ移す。
...
- 完了済みまたは現行仕様に統合済みの計画は、本文から参照し続けるより `docs/product/archive/` または `docs/planning/archive/` へ送る。
