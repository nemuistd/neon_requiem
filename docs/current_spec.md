...
`docs/fiction/world_design.md` で定義された **廻（めぐり）** は正式システム名として扱う。現在の実装では、再固定中枢到達後に任意で廻を実行し、記憶断片を清算して廻後バフを購入し、記録の追記を表示できる。Ch.7 の深層観測所と霞山 澪、Ch.7 補強の深層の静謐、観測記録用の表紙のない日誌、深層観測所 Lv2〜Lv3 記録、澪の2件目イベント、Ch.8 の工学記録保管区、祈念工学実験跡地、七城 皐月、Ch.8 補強の断片の旋律、工学文書の断片（整理済み）、旧評議会・記録の断片、封印された記憶について、澪 / 皐月の接続イベント、Ch.9 起点の再観測拠点、名前のない劇場、白霧 燐、Ch.9 収束補強の最後の名前、バインダー封書・全文、オープンエンド導線、`records_draft.md` 由来の Ch.6〜Ch.9 追加記録整理は最小縦切りとして実装済み。主人公の正体、霞の意志、旧計画の全貌を単一の答えとして確定する演出は未実装であり、長期仕様は `docs/planning/meguri_system_spec.md` と `docs/fiction/meguri_after_design.md` を参照する。
...
| `kasumiyamaMio.oneSongPerDay` | 交流イベント | 霞山 澪 | 廻 1回以上、深層観測所 Lv3、霞山 澪 交流 20 | 一日一曲だけ歌う理由を、観測の変化として語る短い場面 |
| `nanashiroSatsuki.readableUnknown` | 交流イベント | 七城 皐月 | 七城 皐月 交流 5 | 読めることと理解できることを分ける皐月の姿勢を聞く短い場面 |
| `kasumiyamaMio.knownMachine` | 交流イベント | 霞山 澪 | 廻 1回以上、祈念工学実験跡地 Lv1、霞山 澪 交流 20 | 実験跡地の機材と霞のつながりを、確信ではなく既視感として話す短い場面 |
| `nanashiroSatsuki.translationEcho` | 交流イベント | 七城 皐月 | 廻 1回以上、工学記録保管区 Lv5、七城 皐月 交流 20 | 翻訳中の工学記録と夢の言葉が似ている違和感を聞く短い場面 |
...
| `deepLayerSilence` | 深層の静謐 | 深層観測所 Lv3 | 400000 | deepタグ施設の灯るさ生産 x1.25 |
| `fragmentMelody` | 断片の旋律 | 工学記録保管区 Lv3 | 1000000 | 歌・アイテム取得コスト x0.80 |
...
| `coverlessObservationLog` | 観測記録用の表紙のない日誌 | 深層観測所 Lv2 | 200000 | deepタグ施設の灯るさ生産 x1.12 |
| `sortedEngineeringFragment` | 工学文書の断片（整理済み） | 工学記録保管区 Lv2 | 500000 | 歌取得コスト x0.85 |
