...
`docs/fiction/world_design.md` で定義された **廻（めぐり）** は正式システム名として扱う。現在の実装では、再固定中枢到達後に任意で廻を実行し、記憶断片を清算して廻後バフを購入し、記録の追記を表示できる。Ch.7 の深層観測所と霞山 澪、Ch.7 補強の深層の静謐、観測記録用の表紙のない日誌、深層観測所 Lv3〜Lv12 記録、澪の2件目イベント、Ch.8 の工学記録保管区、祈念工学実験跡地、七城 皐月、Ch.8 補強の断片の旋律、工学文書の断片（整理済み）、旧評議会・記録の断片、封印された記憶について、澪 / 皐月の接続イベント、Ch.9 起点の再観測拠点、名前のない劇場、白霧 燐、Ch.9 収束補強の最後の名前、バインダー封書・全文、オープンエンド導線、`records_draft.md` 由来の Ch.6〜Ch.9 追加記録整理は最小縦切りとして実装済み。主人公の正体、霞の意志、旧計画の全貌を単一の答えとして確定する演出は未実装であり、長期仕様は `docs/planning/meguri_system_spec.md` と `docs/fiction/meguri_after_design.md` を参照する。
...
オフライン報酬は、同じ時間のオンライン自然生産を基準にした簡易進行として扱う。基礎効率は `0.5` で、実効値は `現在の秒間灯るさ量 * 離脱秒数 * 0.5 * オフライン報酬倍率` とする。`offline.reward.multiplier` はこの基礎効率に乗算する補助効果である。放置中は施設強化や歌取得による生産力更新が挟まらないため、帰還後に一括投資しても新規施設の開放は最大2段階を目安にする。
...
| `hibikiTooko` | 響木 遠子 | 仮設配信ブース Lv5 | ライブ1回の灯るさ獲得に全施設合計の秒間灯るさ x0.05 を加算 |
| `kaminoMeguri` | 紙野 巡 | 記憶図書館 Lv5 | 全アイドルの交流増加量 x1.25 |
| `hinataKoharu` | 陽向 小春 | 地下広場 Lv5 | 全灯るさ生産 x1.08 / アイテム購入コスト x0.90 |
| `mizukiShino` | 深月 詩乃 | 地下礼拝堂 Lv5 | 全灯るさ生産 x1.10 |
| `tsuginohataSakurako` | 継ノ端 桜子 | 地下通路修復区画 Lv5 | オフライン灯るさ報酬 x1.15 |
| `kasumiyamaMio` | 霞山 澪 | 廻 1回以上 / 深層観測所 Lv5 | deepタグ施設の灯るさ生産 x1.35 |
| `nanashiroSatsuki` | 七城 皐月 | 廻 1回以上 / 工学記録保管区 Lv5 | 歌・アイテム取得コスト x0.85 |
| `shiragiriRin` | 白霧 燐 | 廻 2回以上 / 名前のない劇場 Lv5 | 廻後の施設生産倍率 x1.15 / 記憶断片の獲得見込み +30% |
...
交流イベントは、アイドルとの `bond` が一定値に達した時に表示される短い読み物である。アイドルタブと、左側の注目アイドル枠から開く詳細モーダルで確認できる。記録タブとは別に、アイドル個別状態の `eventIdsRead` で既読を保存する。読んでも灯るさ、生産量、交流値は変化しない。

`交流イベント` は、アイドルとの交流値によって開く個別シーンを指す。記録カテゴリの `アイドルの様子` は、交流値だけでなく施設、廻、章進行などで見えてくるアイドル由来の記録であり、記録タブ側の読み物として扱う。
...
| `kasumiyamaMio.oneSongPerDay` | 交流イベント | 霞山 澪 | 廻 1回以上、深層観測所 Lv8、霞山 澪 交流 20 | 一日一曲だけ歌う理由を、観測の変化として語る短い場面 |
| `nanashiroSatsuki.readableUnknown` | 交流イベント | 七城 皐月 | 七城 皐月 交流 5 | 読めることと理解できることを分ける皐月の姿勢を聞く短い場面 |
| `kasumiyamaMio.knownMachine` | 交流イベント | 霞山 澪 | 廻 1回以上、祈念工学実験跡地 Lv3、霞山 澪 交流 20 | 実験跡地の機材と霞のつながりを、確信ではなく既視感として話す短い場面 |
| `nanashiroSatsuki.translationEcho` | 交流イベント | 七城 皐月 | 廻 1回以上、工学記録保管区 Lv12、七城 皐月 交流 20 | 翻訳中の工学記録と夢の言葉が似ている違和感を聞く短い場面 |
| `shiragiriRin.notFirstMeeting` | 交流イベント | 白霧 燐 | 廻 2回以上、名前のない劇場 Lv3、白霧 燐 交流 5 | 覚えているというより感じている、とだけ返す燐の短い場面 |
...
| `prebroadcastAcapella` | 配信前夜のアカペラ | 仮設配信ブース Lv8 | 6000 | ライブ1回の灯るさ +8 |
| `songOfRecords` | 記録の歌 | 紙野 巡 合流済み / 記憶図書館 Lv10 | 20000 | 施設の灯るさ生産 x1.15 |
| `plazaAnthem` | 広場のアンセム | 陽向 小春 合流済み / 地下広場 Lv10 | 30000 | ライブ1回の灯るさ獲得に全施設合計の灯るさ/秒 x0.08 を加算 |
| `chapelHarmony` | 礼拝堂のハーモニー | 地下礼拝堂 Lv8 | 25000 | 施設の灯るさ生産 x1.10 |
| `twilightChorus` | 薄明のコーラス | 地下礼拝堂 Lv15 | 90000 | 施設の灯るさ生産 x1.25 |
| `restorationHumming` | 修復の仮歌 | 継ノ端 桜子 合流済み / 地下通路修復区画 Lv10 | 80000 | オフライン灯るさ報酬 x1.10 |
| `deepLayerSilence` | 深層の静謐 | 深層観測所 Lv10 | 400000 | deepタグ施設の灯るさ生産 x1.25 |
| `fragmentMelody` | 断片の旋律 | 工学記録保管区 Lv10 | 1000000 | 歌・アイテム取得コスト x0.80 |
| `theLastName` | 最後の名前 | 廻 2回以上 / 白霧 燐 合流済み / 名前のない劇場 Lv10 | 20000000 | 施設の灯るさ生産 x1.30 / 記憶断片の獲得見込み +20% |

「聖歌」は崩壊前の祈念工学上の儀式的・危険な形式を指す深層語であり、通常システム名として軽く扱わない。deep / technical の記録や設定文脈で必要な場合は使用できる。
「修復の仮歌」はオフライン報酬強化の歌であり、地下通路修復区画 Lv15 と合わせて再固定中枢の解放条件になる。
...
| `portableSpotlight` | 携帯スポットライト | 路地裏ステージ Lv8 | 260 | 施設の灯るさ生産 x1.04 |
...
| `coverlessObservationLog` | 観測記録用の表紙のない日誌 | 深層観測所 Lv8 | 200000 | deepタグ施設の灯るさ生産 x1.12 |
| `sortedEngineeringFragment` | 工学文書の断片（整理済み） | 工学記録保管区 Lv8 | 500000 | 歌取得コスト x0.85 |
...
廻実行時に交流 `20` 以上だったアイドルは、`idolRecognition` に痕跡フラグを残す。これは本人が廻を理解している確定描写ではなく、通常UIでは独立した大きな表示を置かず、対象アイドルの交流ラベルを「交流（既視感）」に変える程度に留める。
...
初回廻後の通常進行中も、廻タブは「再固定準備」カードを中心に表示する。ここでは、現在の廻回数、記憶断片、今廻った場合の獲得見込み、次の記憶断片までの値とゲージ、廻実行ボタンを表示する。「引き継ぐもの」と「リセットされるもの」は折りたたみの確認欄に留め、別カードの見取り図として重複表示しない。新しい保存フィールドは追加しない。

初回廻後の通常進行中に未読の追記がある場合だけ、廻タブに短い「追記の更新」導線を表示する。ここでは追記が増えたことと記録タブへ移るボタンだけを示し、記録名、関連バフ名、追記本文は廻タブへ重複表示しない。既読済みの追記だけになった場合、この導線は表示しない。
...
Ch.9 収束補強では、廻2以降、名前のない劇場 Lv15 と「最後の名前」取得後に廻タブへ「再び灯るものへ」の導線を表示する。これは物語上の解決ではなく、復興を続けるか、もう一度廻るかを選べるオープンエンド表示として扱う。新しい保存フィールドは使わず、`meguri.count`、施設 Lv、歌取得状態から判定する。
...
| 30分 | ネオン掲示板を複数回強化し、交代用連絡ボード、録音済みの短い挨拶、朝霧 結の解放に届く。プレイ密度が高ければネオン掲示板 Lv15 から薄明通り案内所の解放も視野に入る。 |

## 記録

記録は進行条件に応じて解放され、既読状態を保存する。数値効果は持たない。`アイドルの様子` カテゴリは、アイドルに関係する場面や変化を記録タブに残すための分類であり、交流値、施設、廻、章進行など複数の条件で解放される。`アイドルの様子` は `relatedIdolId` で対象アイドルを示し、解放済みのものは記録タブに加えてアイドル詳細モーダルからも確認できる。廻後バフ取得後、一部の既存記録には本文差し替えではなく末尾追記が薄い別色で表示され、追記の既読状態も保存する。廻で既読記録が引き継がれただけでは記録タブ通知を出さず、追記が新しく表示された場合は追記未読として通知する。
...
surface の記録に、祈念工学、アンカー、聖歌、祈念負荷を出さない。これは「聖歌」という存在を避ける意味ではなく、崩壊前の祈念工学上の儀式的・危険な形式を通常システム名として軽く扱わないための制約である。
...
灯るさ、価格、生産量、毎秒値などの大きな数値は、10万以上で `万` / `億` / `兆` の省略表示を使う。詳細値はゲーム内の小さな詳細ポップアップに残し、マウスホバーやキーボードフォーカスで確認できるようにする。
