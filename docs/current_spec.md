...
記録は進行条件に応じて解放され、既読状態を保存する。数値効果は持たない。`アイドルの様子` カテゴリは、アイドルに関係する場面や変化を記録タブに残すための分類であり、交流値、施設、廻、章進行など複数の条件で解放される。`アイドルの様子` は `relatedIdolId` で対象アイドルを示し、解放済みのものは記録タブに加えて左側の注目アイドル詳細パネルからも確認できる。廻後バフ取得後、一部の既存記録には本文差し替えではなく末尾追記が薄い別色で表示され、追記の既読状態も保存する。廻で既読記録が引き継がれただけでは記録タブ通知を出さず、追記が新しく表示された場合は追記未読として通知する。
