...
`docs/fiction/world_design.md` で定義された **廻（めぐり）** は正式システム名として扱う。ただし、現在の実装にはまだ廻システム本体、記憶断片リソース、廻後バフ画面、廻後アノテーションは入っていない。実装前仕様は `docs/planning/meguri_system_spec.md` を参照する。
...
地下通路修復区画は Chapter 5 末尾の深層接続施設として扱う。次の `再固定中枢` は廻本体の起点になるため、`docs/planning/meguri_system_spec.md` の実装前仕様に沿って別タスクで実装する。
...
「聖歌」は通常システム名にしない。
「修復の仮歌」は現時点ではオフライン報酬強化の歌であり、将来の廻解放条件候補として扱う。廻本体を実装するまでは、廻発動処理や記憶断片リソースには接続しない。
